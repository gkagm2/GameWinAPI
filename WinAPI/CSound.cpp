...
{
	if (nullptr == CSoundManager::GetInstance()->GetSoundDevice())
		assert(nullptr); // 사운드 객체 생성 안됨
...
bool CSound::LoadWaveSound(const wstring& _strPath)
{
	...
	if (FAILED(CSoundManager::GetInstance()->GetSoundDevice()->CreateSoundBuffer(&m_tBuffInfo, &m_pSoundBuffer, NULL)))
	{
		MessageBox(NULL, L"사운드버퍼 생성 실패", L"", MB_OK);
		return false;
	}
...
{
	CSoundManager::GetInstance()->RegisterToBGM(this);
